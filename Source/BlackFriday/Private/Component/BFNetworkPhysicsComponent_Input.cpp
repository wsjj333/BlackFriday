...
void UBFNetworkPhysicsComponent::ApplyStateWithTeleportCheck(const FBFPhysicsState& NewState)
{
	...
	if (Prim)
	{
		float DistSq = FVector::DistSquared(TargetState.Pos, Prim->GetComponentLocation());
		if (DistSq > TeleportDist * TeleportDist)
		{
			Prim->SetWorldLocationAndRotation(TargetState.Pos, TargetState.Rot, false, nullptr, ETeleportType::TeleportPhysics);
			SmoothAlpha = 1.0f;
		}
	}
}
